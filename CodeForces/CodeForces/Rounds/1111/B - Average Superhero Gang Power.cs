...
public class B___Average_Superhero_Gang_Power
{
    private static void Solve()
    {
        ...
        int k = ReadInt();
        int m = ReadInt();
        var heroes = ReadIntArray().ToList();

        heroes.Sort();

        // A - Remove all lower than max and maximize max
        double optionA = 0;
        var filtered = new List<int>();
        int lengthExceptLast = heroes.Count - 1;
        var heroesArray = new int[n];
        heroes.CopyTo(heroesArray);

        var heroesCopyList = heroesArray.ToList();

        //heroesCopyList.RemoveRange(0, Min(lengthExceptLast, m));
        //m -= Min(lengthExceptLast, m);

        int j = 0;
        while (j < n - 1 && m > 0)
        {
            int cur = heroesArray[j];
            if (cur >= heroesArray[n - 1]) // n - 1 is max
                break;

            heroesCopyList.Remove(cur);
        ...
        // Maximize max
        int max = heroesCopyList.Last() + Min(m, k);
        m -= Min(m, k);

        // Remove items after k == 0 but m > 0
        while (m > 0 && heroesCopyList.Count > 1)
        {
            heroesCopyList.Remove(heroesCopyList.First());
            m--;
        }

        int[] hArray = heroesCopyList.ToArray();
        for (int i = 0; i < hArray.Length - 1; i++) // Not last one
        {
            optionA += hArray[i];
        }

        optionA = (optionA + max) / hArray.Length;

        

        // Edge case: What if multiple times same? (3, 3, 3, 3)
        // -> Always maximize
        double optionB = 0;

        Write(Max(optionA, optionB));
    }
